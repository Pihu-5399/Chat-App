...
export const userLogin = async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.status(400).send({ err: "Provide all fields before login" })
        }
        else {
...
export const getUser = async (req, res) => {
    try {
        ...
        if (!userDetails) {
            return res.status(401).send({ error: "User is not available" })
        }
        else res.status(200).send(userDetails)
    } catch (err) {
        return res.status(500).send({ error: "Something went wrong", errorMsg: err.message })
    }
}
